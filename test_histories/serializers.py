...
from tests.models import ProductiveTest, ReceptiveTest, ReceptiveQuestion, ReceptiveAnswer
...
class ReceptiveAnswerHistorySerializer(serializers.Serializer):
    ...
    receptive_answer = serializers.PrimaryKeyRelatedField(
        queryset=ReceptiveAnswer.objects.all(),
        required=False,
        allow_null=True
    )
    ...
    def validate(self, attrs):
        ...
        # Matching formats (E,J) - must have receptive_answer
            ...
            if receptive_answer.receptive_question.receptive_part_id != receptive_question.receptive_part_id:
                raise serializers.ValidationError({
...
class ReceptiveTestHistorySerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if not self.instance:
            ...
            # Check if receptive_test exists
    ...
    def _calculate_answer_score(self, question, selected_answer=None, user_text=None):
        ...
        if part_format in ['A', 'B', 'C', 'E', 'F', 'G', 'H', 'J']:
            ...
            if (selected_answer.receptive_question_id == question.id and 
    ...
    def create(self, validated_data):
        ...
        if not student:
            raise serializers.ValidationError({"student": "Student is required in context."})
...
class ReceptiveTestHistoryDetailSerializer(serializers.ModelSerializer):
    ...
    test_title = serializers.CharField(source='receptive_test.test.title', read_only=True)
        ...
        return [
            {
                ...
                'selected_answer_id': ah.receptive_answer.id if ah.receptive_answer else None,
