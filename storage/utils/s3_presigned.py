...
class S3PresignedURLManager:
    ...
    ALLOWED_MIME_TYPES = {
        ...
        'tests': ['image/jpeg', 'image/png', 'video/mp4', 'audio/mpeg'],
    }
    ...
    def __init__(self):
        ...
        self.endpoint_url = getattr(settings, 'AWS_S3_ENDPOINT_URL', 'http://minio:9000')
        self.client_endpoint_url = getattr(settings, 'AWS_S3_CLIENT_ENDPOINT_URL', 'http://localhost:9000')
    ...
    def generate_presigned_post(self, request=None, key: str = None, file_size: int = None, mime_type: str = None):
        """
        ...
        if not self.validate_file_size(file_size):
            raise ValueError(f'File size must be between 1 and {self.MAX_FILE_SIZE} bytes')
    ...
    def generate_file_key(self, category: str, user_id, filename: str, test_id=None, part=None) -> str:
        """
